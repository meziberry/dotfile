;;; autoload/auto-files.el -*- lexical-binding: t; -*-

(defun radian--resolve-path-forms (spec &optional directory)
  "Converts a simple nested series of or/and forms into a series of
`file-exists-p' checks.

For example

  (radian--resolve-path-forms
    '(or A (and B C))
    \"~\")
...
This is used by `file-exists-p!' and `project-file-exists-p!'."
  ...
  (if (and (listp spec)
      ...
      (cons (car spec)
            (mapcar (radian-rpartial #'radian--resolve-path-forms directory)
                    (cdr spec)))
...
;;;###autoload
(defun radian-path (&rest segments)
    ...
    (while segments
      (setq segment (pop segments)
            dir (expand-file-name
                 (if (listp segment)
                     (apply #'radian-path dir segment)
                   segment)
                 dir)))
    dir))

;;;###autoload
(defun radian-glob (&rest segments)
  "Construct a path from SEGMENTS and expand glob patterns.
Returns nil if the path doesn't exist.
Ignores `nil' elements in SEGMENTS."
  (let (case-fold-search)
    (file-expand-wildcards (apply #'radian-path segments) t)))

;;;###autoload
(defun radian-dir (&rest segments)
  "Constructs a path from SEGMENTS.
See `radian-path'.
Ignores `nil' elements in SEGMENTS."
  (when-let (path (radian-path segments))
    (directory-file-name path)))

;;;###autoload
(cl-defun radian-files-in
    (paths &rest rest
           &key
           filter
           map
...
;;;###autoload
(defun radian-file-cookie-p (file &optional cookie null-value)
...
single file or nested compound statement of `and' and `or' statements."
  `(let ((p ,(radian--resolve-path-forms files directory)))
     (and p (expand-file-name p ,directory))))

;;;###autoload
(defun radian-file-size (file &optional dir)
...
;;;###autoload
(defun radian-directory-size (dir)
  "Returns the size of FILE (in DIR) in kilobytes."
  (unless (file-directory-p dir)
    (error "Directory %S does not exist" dir))
  (if (executable-find "du")
      (/ (string-to-number (cdr (radian-call-process "du" "-sb" dir)))
         1024.0)
        ...
        (unless (member (car attrs) '("." ".."))
          (cl-incf
           sum (if (eq (nth 1 attrs) t) ; is directory
                   (radian-directory-size (expand-file-name (car attrs) dir))
                 (/ (nth 8 attrs) 1024.0))))))))
...
(defun radian--update-files (&rest files)
    ...
    (dolist (file files)
      ...
      (unless (file-readable-p file)
        ...
        (when (and (bound-and-true-p projectile-mode)
                   (radian-project-p)
                   (projectile-file-cached-p file (radian-project-root)))
...
;;;###autoload
(defun radian/delete-this-file (&optional path force-p)
    ...
    (let ((buf (current-buffer)))
      (unwind-protect
        ...
        (if (file-exists-p path)
          ...
          ;; real buffers (`radian-real-buffer-p')
          (radian/kill-this-buffer-in-all-windows buf t)
          (radian--update-files path)
          (message "Deleted %S" short-path))))))

;;;###autoload
(defun radian/copy-this-file (new-path &optional force-p)
...
If FORCE-P, overwrite the destination file if it exists, without confirmation."
  ...
  (let ((old-path (buffer-file-name (buffer-base-buffer)))
    ...
    (radian--update-files old-path new-path)
    (message "File copied to %S" (abbreviate-file-name new-path))))

;;;###autoload
(defun radian/move-this-file (new-path &optional force-p)
...
(defun radian--sudo-file-path (file)
...
;;;###autoload
(defun radian/sudo-find-file (file)
  "Open FILE as root."
  (interactive "FOpen file as root: ")
  (find-file (radian--sudo-file-path file)))

;;;###autoload
(defun radian/sudo-this-file ()
  "Open the current file as root."
  (interactive)
  (find-file
   (radian--sudo-file-path
    (or buffer-file-name
...
;;;###autoload
(defun radian/sudo-save-buffer ()
  "Save this file as root."
  (interactive)
  (let ((file (radian--sudo-file-path buffer-file-name)))
...
;;;###autoload
(defun radian/remove-recent-file (file)
