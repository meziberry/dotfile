...
Made for `org-tab-first-hook' in meow-mode."
  (interactive)
  (cond ((not (and (bound-and-true-p meow-mode)
                   (meow-insert-mode-p)))
         nil)
        ((org-at-item-p)
...
Made for `org-tab-first-hook'."
  (when (and (featurep! 'yasnippet)
    ...
    (and (let ((major-mode (cond ((org-in-src-block-p t)
           ...
           (cond ((and (or (not (bound-and-true-p meow-mode))
                           (meow-insert-mode-p)
                           (meow-motion-mode-p))
...
`org-show-subtree' , and `org-cycle' on shift-TAB if I need it."
  (interactive "P")
...
(defun +org-enable-auto-update-cookies-h ()
  "Update statistics cookies when saving or exiting insert mode (`meow-mode')."
  (when (bound-and-true-p meow-mode)
    (add-hook 'meow-insert-exit-hook #'org-update-parent-todo-statistics nil t))
