...
;;;###autoload (autoload 'org-roam-node-radian-type "lang/org/autoload/contrib-roam2" nil t)
(cl-defmethod org-roam-node-radian-type ((node org-roam-node))
...
Intended to be added to `radian-switch-buffer-hook' in `org-roam-find-file-hook'.
Controlled by `+org-roam-open-buffer-on-find-file'."
  (when (and +org-roam-auto-backlinks-buffer
    ...
    (let ((visible-p (eq 'visible (org-roam-buffer--visibility))))
      (if (cl-some #'org-roam-buffer-p (radian-visible-buffers))
          (unless visible-p
            (org-roam-buffer-toggle))
        (when visible-p
          (org-roam-buffer-toggle))
        (unless (radian-buffers-in-mode 'org-mode)
          (remove-hook 'radian-switch-buffer-hook #'+org-roam-manage-backlinks-buffer-h))))))
