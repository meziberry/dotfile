;;; autoload/output.el -*- lexical-binding: t; -*-

(defvar radian-output-ansi-alist
  '(;; fx
    (bold       1 :weight bold)
    (dark       2)
...
(defvar radian-output-class-alist
  `((color . radian--output-color)
    (class . radian--output-class)
    (indent . radian--output-indent)
    (autofill . radian--output-autofill)
    (success . (lambda (str &rest args)
                 (apply #'radian--output-color 'green (format "✓ %s" str) args)))
    (warn    . (lambda (str &rest args)
                 (apply #'radian--output-color 'yellow (format "! %s" str) args)))
    (error   . (lambda (str &rest args)
                 (apply #'radian--output-color 'red (format "x %s" str) args)))
    ...
    (debug   . (lambda (str &rest args)
                 (if radian-debug-p
                     (apply #'radian--output-color 'dark
                            (format "- %s" str)
                            args)
                   "")))
...
(defvar radian-output-indent 0
  "Level to rigidly indent text returned by `format!' and `print!'.")

(defvar radian-output-indent-increment 2
  "Steps in which to increment `radian-output-indent' for consecutive levels.")

(defvar radian-output-backend
  (if radian-interactive-p 'text-properties 'ansi)
...
;;;###autoload
(defun radian--format (output)
  (if (string-empty-p (string-trim output))
      ""
    (concat (make-string radian-output-indent 32)
            (replace-regexp-in-string
             "\n" (concat "\n" (make-string radian-output-indent 32))
             output t t))))

;;;###autoload
(defun radian--print (output)
...
;;;###autoload
(defun radian--output-indent (width text &optional prefix)
...
;;;###autoload
(defun radian--output-autofill (&rest msgs)
      ...
      (`text-properties
       ...
       (propertize
        ...
        (append (get-text-property 0 'face format)
                (cond ((>= code 40)
                       `(:background ,(caddr (assq style radian-output-ansi-alist))))
                      ((>= code 30)
                       `(:foreground ,(face-foreground (caddr (assq style radian-output-ansi-alist)))))
                      ((cddr (assq style radian-output-ansi-alist)))))))
      (_ message))))

;;;###autoload
(defun radian--output-class (class format &rest args)
  "Apply CLASS to formatted format with ARGS.

CLASS is derived from `radian-output-class-alist', and can contain any arbitrary,
transformative logic."
  (let (fn)
    (cond ((setq fn (cdr (assq class radian-output-class-alist)))
           (if (functionp fn)
...
;;;###autoload
(defun radian--output-apply (forms &optional sub)
  "Replace color-name functions with calls to `radian--output-color'."
  (cond ((null forms) nil)
        ((listp forms)
         (append (cond ((not (symbolp (car forms)))
                        (list (radian--output-apply (car forms))))
                       (sub
                        (list (car forms)))
                       ((assq (car forms) radian-output-ansi-alist)
                        `(radian--output-color ',(car forms)))
                       ((assq (car forms) radian-output-class-alist)
                        `(radian--output-class ',(car forms)))
                       ((list (car forms))))
                 (radian--output-apply (cdr forms) t)
                 nil))
        (forms)))
...
into faces or ANSI codes depending on the type of sesssion we're in."
  `(radian--format (format ,@(radian--output-apply `(,message ,@args)))))

;;;###autoload
(defmacro print-group! (&rest body)
  "Indents any `print!' or `format!' output within BODY."
  `(let ((radian-output-indent (+ radian-output-indent-increment radian-output-indent)))
     ,@body))
...
Uses faces in interactive sessions and ANSI codes otherwise."
  `(radian--print (format! ,message ,@args)))
...
DEST can be one or more of `standard-output', a buffer, a file"
  (declare (indent 1))
  `(let* ((log-buffer (generate-new-buffer " *radian log*"))
          (standard-output
           (lambda (out)
     ...
     (letf! (defun message (msg &rest args)
              (when msg
                (print-group!
                 (with-current-buffer log-buffer
                   (insert (radian--format (apply #'format msg args)) "\n"))
                 (when (or radian-debug-p (not inhibit-message))
                   (radian--print (radian--format (apply #'format msg args))))))
              message)
       (unwind-protect
           ,(macroexp-progn body)
         (with-current-buffer log-buffer
