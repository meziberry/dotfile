...
(defun radian--advice-fix-xw-display-color-p (func &optional display)
  ...
  (if (or display after-init-time)
    ...
    initial-window-system))
(advice-add #'xw-display-color-p :around #'radian--advice-fix-xw-display-color-p)
...
resources.")
(advice-add #'x-apply-session-resources :override #'radian--advice-disable-x-resource-application)
