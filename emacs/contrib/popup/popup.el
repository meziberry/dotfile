...
(defvar +popup-buffer-mode-map
  (let ((map (make-sparse-keymap)))
    (when (featurep! 'meow)
      ...
      (define-key map [escape] #'radian/escape))
    map)
  "Active keymap in popup windows. See `+popup-buffer-mode'.")

(define-minor-mode +popup-mode
  "Global minor mode representing Radian's popup management system."
  :init-value nil
  :global t
  :keymap +popup-mode-map
  (cond (+popup-mode
         (add-hook 'radian-escape-hook #'+popup-close-on-escape-h 'append)
        ...
        (t
         (remove-hook 'radian-escape-hook #'+popup-close-on-escape-h)
...
(set-popup-rules!
 ...
 '(("^\\*Completions" :ignore t)
   ...
   ("^\\*\\(?:radian \\|Pp E\\)"  ; transient buffers (no interaction required)
    :vslot -3 :size +popup-shrink-to-fit :autosave t :select ignore :quit t :ttl 0)
   ("^\\*radian:"  ; editing buffers (interaction required)
    :vslot -4 :size 0.35 :autosave t :select t :modeline t :quit nil :ttl t)
   ("^\\*radian:\\(?:v?term\\|e?shell\\)-popup"  ; editing buffers (interaction required)
...
(add-hook 'radian-init-ui-hook #'+popup-mode 'append)
